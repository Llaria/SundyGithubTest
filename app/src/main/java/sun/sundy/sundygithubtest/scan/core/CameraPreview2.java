...
public class CameraPreview2 extends SurfaceView implements SurfaceHolder.Callback {
    ...
    void onScanBoxRectChanged(Rect scanRect) {
        ...
        SundyLogUtil.printRect("转换前", scanRect);

        if (SundyLogUtil.isPortrait(getContext())) {
            int temp = centerX;
            centerX = centerY;
            centerY = temp;
        ...
        SundyLogUtil.printRect("转换后", scanRect);

        SundyLogUtil.d("扫码框发生变化触发对焦测光");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() == 1 && (event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
            ...
            int focusSize = SundyLogUtil.dp2px(getContext(), 120);
        ...
        if (event.getPointerCount() == 2) {
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_POINTER_DOWN:
                    mOldDist = SundyLogUtil.calculateFingerSpacing(event);
                    break;
                case MotionEvent.ACTION_MOVE:
                    float newDist = SundyLogUtil.calculateFingerSpacing(event);
                    if (newDist > mOldDist) {
    ...
    private static void handleZoom(boolean isZoomIn, Camera camera) {
        ...
        if (params.isZoomSupported()) {
            ...
            if (isZoomIn && zoom < params.getMaxZoom()) {
                ...
                SundyLogUtil.d("既不放大也不缩小");
            }
            params.setZoom(zoom);
            camera.setParameters(params);
        } else {
            SundyLogUtil.d("不支持缩放");
        }
    }
    ...
    private void handleFocusMetering(float originFocusCenterX, float originFocusCenterY,
        ...
        try {
            ...
            if (focusMeteringParameters.getMaxNumFocusAreas() > 0) {
                SundyLogUtil.d("支持设置对焦区域");
                isNeedUpdate = true;
                Rect focusRect = SundyLogUtil.calculateFocusMeteringArea(1f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                SundyLogUtil.printRect("对焦区域", focusRect);
                ...
                SundyLogUtil.d("不支持设置对焦区域");
            }

            if (focusMeteringParameters.getMaxNumMeteringAreas() > 0) {
                SundyLogUtil.d("支持设置测光区域");
                isNeedUpdate = true;
                Rect meteringRect = SundyLogUtil.calculateFocusMeteringArea(1.5f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                SundyLogUtil.printRect("测光区域", meteringRect);
                focusMeteringParameters.setMeteringAreas(Collections.singletonList(new Camera.Area(meteringRect, 1000)));
            } else {
                SundyLogUtil.d("不支持设置测光区域");
            }
            ...
            if (isNeedUpdate) {
                ...
                mCamera.autoFocus(new Camera.AutoFocusCallback() {
                    public void onAutoFocus(boolean success, Camera camera) {
                        if (success) {
                            SundyLogUtil.d("对焦测光成功");
                        } else {
                            SundyLogUtil.e("对焦测光失败");
                        }
                        startContinuousAutoFocus();
                    }
                });
            ...
            e.printStackTrace();
            SundyLogUtil.e("对焦测光失败：" + e.getMessage());
            startContinuousAutoFocus();
        }
    }
