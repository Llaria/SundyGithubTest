...
class ProcessDataTask extends AsyncTask<Void, Void, ScanResult> {
    ...
    protected ScanResult doInBackground(Void... params) {
        ...
        if (mPicturePath != null) {
            return qrCodeView.processBitmapData(SundyLogUtil.getDecodeAbleBitmap(mPicturePath));
...
//                SundyLogUtil.d("两次任务执行的时间间隔：" + (System.currentTimeMillis() - sLastStartTime));
            ...
            if (SundyLogUtil.isDebug()) {
                ...
                if (scanResult != null && !TextUtils.isEmpty(scanResult.result)) {
                    SundyLogUtil.d("识别成功时间为：" + time);
                } else {
                    SundyLogUtil.e("识别失败时间为：" + time);
                }
            }
